...
const VideoTitle = ({ movieId, title, language, isAdult, rating, thumbnail, currentIndex }) => {
  ...
  const movieLogo = trailer?.[movieId]?.movie_logo ? (
    ...
    <div className="w-20 md:w-72 text-xs md:text-lg">{title}</div>
  );
    ...
    <div
      ...
      <div>
        <div className="flex items-center text-xs md:text-lg">
          <div className="bg-yellow-500 rounded-sm px-1 text-black font-extrabold mr-2">TMDb</div>
          {rating ? rating.toFixed(1) : ""}
          <Star color="#f4d50b" fill="#f4d50b" strokeWidth={3} className="ml-2 w-4" />
        </div>
        ...
        <div className="mt-2 md:mt-12 lg:md-6 flex items-center">
          {isTrailerAvailable ? (
            <span className="flex items-center text-xs md:text-lg">
              <Check strokeWidth={5} color="#000000" className="bg-sky-500 h-4 rounded-full p-1 mr-2" />
              Included with Prime
            </span>
          ) : (
            <span className="flex items-center text-xs md:text-lg">
              <ShoppingBagIcon strokeWidth={2} color="#fbbf24" className="h-4 mr-2" />
              Available to rent
            </span>
          )}
          {isTrailerAvailable && <div className="pl-5 text-sm md:text-xl flex items-center">Play Trailer</div>}
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger>
            ...
            <Tooltip>
              <TooltipTrigger>
                <div className="bg-neutral-700 rounded-full p-2 md:p-3 w-fit h-fit ms-2 cursor-pointer">
                  <Info color="#ffffff" strokeWidth={2} className="h-8 w-8" />
                </div>
              </TooltipTrigger>
              <TooltipContent>
